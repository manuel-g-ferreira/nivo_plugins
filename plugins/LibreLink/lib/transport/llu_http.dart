...
import 'package:nivo_plugins/persistent_http_client.dart';

/// Low-level HTTP to regional LibreLink Up hosts — no business logic.
class LluHttp with PersistentHttpClient {
  LluHttp({
  ...
  Future<Map<String, dynamic>> get(
    ...
    try {
      final request = await httpClient.getUrl(uri(path));
  ...
  Future<Map<String, dynamic>> post(
    ...
    try {
      final request = await httpClient.postUrl(uri(path));
      headers(authenticated: authenticated).forEach(request.headers.set);
