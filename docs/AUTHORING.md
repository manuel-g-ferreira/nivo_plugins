...
Ship a **`.nivo`** package (directory bundle, not zip). Build with:

```bash
dart run tool/glucose_plugin.dart build plugins/YourPlugin
```

End users drag the package into **Settings → Plugins** or use **Choose .nivo…**.

## Official references

- [PLUGIN-PROTOCOL-V1.md](PLUGIN-PROTOCOL-V1.md)
- [PLUGIN-MANIFEST.md](PLUGIN-MANIFEST.md)
- Reference implementations: `plugins/MockCGM`, `plugins/LibreLink`, `plugins/Nightscout`, `plugins/DexcomShare`
