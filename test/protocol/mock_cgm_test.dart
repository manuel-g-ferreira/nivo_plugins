...
void main() {
  ...
  test('fetchReadings honors options.historySince for delta history', () async {
    final process = PluginProcess(executablePath);
    await process.start();
    addTearDown(process.terminate);

    final auth = await process.send({'command': 'authenticate'});
    final full = await process.send({
      'command': 'fetchReadings',
      'authToken': auth['authToken'],
      'userId': auth['userId'],
      'dataSourceId': auth['defaultDataSourceId'],
    ...
    final fullHistory = full['history'] as List<dynamic>;
    expect(fullHistory.length, greaterThan(2));

    final pivot = DateTime.parse(
      fullHistory[fullHistory.length ~/ 2]['timestamp'] as String,
    );
    final delta = await process.send({
      'command': 'fetchReadings',
      'authToken': auth['authToken'],
      'userId': auth['userId'],
      'dataSourceId': auth['defaultDataSourceId'],
      'hours': 3,
      'options': {'historySince': pivot.toUtc().toIso8601String()},
    }, timeout: const Duration(seconds: 30));

    expect(delta['success'], true);
    final deltaHistory = delta['history'] as List<dynamic>;
    expect(deltaHistory.length, lessThan(fullHistory.length));
    for (final reading in deltaHistory) {
      final ts = DateTime.parse(reading['timestamp'] as String);
      expect(ts.isAfter(pivot), isTrue);
    }
  });
}
