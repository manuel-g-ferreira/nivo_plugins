...
```json
{
  ...
  "hours": 3,
  "options": {
    "historySince": "2026-05-30T12:00:00.000Z"
  }
}
```

When `options.historySince` is present, return readings with timestamps **strictly after** that value in `history` (plus `current`). The host sets this per poll from in-memory history; it is not persisted in credentials.
...
- [ ] Vendor HTTP mapping documented in `docs/<vendor>-api-analysis.md` (optional)
